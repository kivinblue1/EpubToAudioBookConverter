...
class EpubAudioConverterUI(tk.Tk):
    ...
    def __init__(self):
        ...
        self.language_prefix_var = tk.StringVar(value="ru")
        ...
        # New: Language Prefix Input
        row_number += 1
        self.language_prefix_label = self._create_label("Префикс языка, введите например en или fr:", row_number)
        self.language_prefix_entry = self._create_entry(self.language_prefix_var, row_number)
        self.refresh_voices_button = self._create_button(i18n._("refresh_voices"), row_number, self._refresh_voices_list)

        # Language Dropdown
        row_number += 1
        self.voice_label = self._create_label(i18n._("voice"), row_number)
        self.voice_option = sorted(my_edge_tts.load_voices(self.language_prefix_var.get()))
        ...
        # Language Selector
        row_number += 1
        self.language_label = self._create_label(i18n._("language"), row_number)
        self.language_dropdown = ctk.CTkOptionMenu(self.main_frame, values=["EN", "RU"], variable=self.language_var, command=self._on_language_change)
        self.language_dropdown.grid(row=row_number, column=1, sticky="we")

        # Output Directory Path
        row_number += 1
        self.output_directory_label = self._create_label(i18n._("output_directory"), row_number)
        self.output_directory_entry = self._create_entry(self.output_directory_path, row_number)
        self.output_directory_button = self._create_button(i18n._("browse"), row_number, self._browse_output_directory)

        # Output File Name
        row_number += 1
        self.output_file_label = self._create_label(i18n._("output_file_name"), row_number)
        self.output_file_entry = self._create_entry(self.output_file_name, row_number)
        self.output_in_one_file = customtkinter.CTkCheckBox(self.main_frame, text=i18n._("single_mp3_file"),
        ...
        row_number += 1
        self.playback_speed_label = self._create_label(i18n._("playback_speed"), row_number)
        ...
        # Volume Percentage
        row_number += 1
        self.volume_label = self._create_label(i18n._("volume"), row_number)
        ...
        # Pitch Hz
        row_number += 1
        self.pitch_label = self._create_label(i18n._("pitch"), row_number)
        ...
        # Book title
        row_number += 1
        self.book_title_label = self._create_label(i18n._("book_title"), row_number)
        self.book_title_value_label = self._create_entry(self.book_title, row_number)
        self.select_all_button = self._create_button(i18n._("select_all"), row_number, self._select_all_chapters)
        ...
        self.chapter_list_label = self._create_label(i18n._("chapter_list"), row_number)
        self.chapter_completed = set()
        ...
        self.unselect_all_button = self._create_button(i18n._("unselect_all"), row_number, self._unselect_all_chapters, "ne")
        self.reset_progress_button = self._create_button(i18n._("progress_reset"), row_number, self._reset_progress, "w", columnspan=1)

        # Selected Chapter Text
        row_number += 1
        self.selected_chapter_label = self._create_label(i18n._("selected_chapter_text"), row_number)
        ...
        # Buttons
        row_number += 1
        self.generate_button = self._create_button(i18n._("generate"), row_number, self._generate, column=0, sticky="ew", columnspan=3)
        ...
        self.progress_bar.grid(row=row_number + 1, column=0, columnspan=3, sticky="ew", padx=self.default_pad_x, pady=self.default_pad_y)
    ...
    def _configure_grid(self):
        ...
        self.main_frame.grid_rowconfigure(9, weight=1) # Adjusted for new row
        self.main_frame.grid_rowconfigure(10, weight=1) # Adjusted for new row
        self.main_frame.grid_columnconfigure(1, weight=1, pad=10)

    def _refresh_voices_list(self):
        self._log(i18n._("updating_voices"))
        current_language_prefix = self.language_prefix_var.get()
        new_voices = sorted(my_edge_tts.load_voices(current_language_prefix))

        if not new_voices:
            self._log(i18n._("no_voices_error").format(prefix=current_language_prefix))
            return

        self.voice_dropdown.configure(values=new_voices)
        if new_voices:
            self.voice_var.set(new_voices[0])
        else:
            self.voice_var.set("")
        self._log(i18n._("voices_loaded"))

    def _on_language_change(self, choice):
        lang_map = {"EN": "en", "RU": "ru"}
        i18n.set_language(lang_map.get(choice, "en"))
        self._refresh_ui_texts()

    def _refresh_ui_texts(self):
        self.epub_file_label.configure(text=i18n._("epub_file_path"))
        self.epub_file_button.configure(text=i18n._("browse"))
        self.language_prefix_label.configure(text=i18n._("language_prefix"))
        self.refresh_voices_button.configure(text=i18n._("refresh_voices"))
        self.voice_label.configure(text=i18n._("voice"))
        self.language_label.configure(text=i18n._("language"))
        self.output_directory_label.configure(text=i18n._("output_directory"))
        self.output_directory_button.configure(text=i18n._("browse"))
        self.output_file_label.configure(text=i18n._("output_file_name"))
        self.output_in_one_file.configure(text=i18n._("single_mp3_file"))
        self.playback_speed_label.configure(text=i18n._("playback_speed"))
        self.volume_label.configure(text=i18n._("volume"))
        self.pitch_label.configure(text=i18n._("pitch"))
        self.book_title_label.configure(text=i18n._("book_title"))
        self.select_all_button.configure(text=i18n._("select_all"))
        self.chapter_list_label.configure(text=i18n._("chapter_list"))
        self.unselect_all_button.configure(text=i18n._("unselect_all"))
        self.selected_chapter_label.configure(text=i18n._("selected_chapter_text"))
        self.generate_button.configure(text=i18n._("generate"))
    ...
    def _browse_epub_file(self):
        file_path = filedialog.askopenfilename(filetypes=[("EPUB files", "*.epub")],
                                               initialdir=os.path.dirname(self.epub_file_path.get()) if self.epub_file_path.get() else None)
        if file_path:
            self.epub_file_path.set(os.path.normpath(file_path))

    def _browse_output_directory(self):
        directory_path = filedialog.askdirectory(initialdir=self.output_directory_path.get() if self.output_directory_path.get() else None)
        if directory_path:
            self.output_directory_path.set(os.path.normpath(directory_path))
    ...
    def _generate(self):
        ...
        self.log_label.configure(text='''''') # Clear log
        ...
        if self.output_in_one_file_var.get() == "on":
            ...
            for index, chapter_file in zip(
                ...
                self._generate_one_mp3_file(chapter_text, file_name, i, total, chapter_file)

        file_management.open_directory_in_explorer(self.output_directory_path.get())
        self.progress_bar.grid_remove()  # Hide progress bar
        self._log(i18n._("audio_book_created"))

    def _generate_one_mp3_file(self, text, file_name, index, total, chapter_file=None):
        ...
        if chapter_file is None:
            ...
            mp3_file_name = f"{file_name}-{chapter_name}.mp3"

        mp3_file_path = os.path.normpath(os.path.join(self.output_directory_path.get(), mp3_file_name))

        self._log(i18n._("generating").format(index=str(index).zfill(3), total=str(total).zfill(3), file=mp3_file_path))

        success = my_edge_tts.generate_mp3_file(
            temporary_file_path,
            mp3_file_path,
            self.voice_var.get(),
        ...
        if not success:
            ...
            self._log(i18n._("chapter_failed").format(chapter=chapter_file or file_name))
            return False

        if chapter_file is not None:
            progress_tracker.mark_chapter_completed(self.epub_file_path.get(), chapter_file)
            self._mark_chapter_completed_in_listbox(chapter_file)
            self._log(i18n._("chapter_completed").format(chapter=chapter_file))
    ...
    def load_ui_status(self):
        try:
            with open("../ui_status.json", "r") as file:
                ...
                self.language_prefix_var.set(ui_status.get("language_prefix_var", "ru"))
                ...
                language = ui_status.get("language", "ru")
                self.language_var.set("RU" if language == "ru" else "EN")
    ...
    def save_ui_status(self):
        ...
        ui_status = {
            ...
            "language_prefix_var": self.language_prefix_var.get(),
            ...
            "language": lang_map.get(self.language_var.get(), "ru")
        }
    ...
    def _delete_chapter_list(self):
        if self.chapter_listbox.size() > 0:
            ...
            self._log(i18n._("deleting_chapter_list"))
    ...
    def _fill_chapter_list(self, chapters_filename_and_text):
        ...
        self._log(i18n._("filling_chapter_list"))

        sorted_chapters = sorted(chapters_filename_and_text, key=lambda x: x[0])
        for file_name, text in sorted_chapters:
    ...
    def load_epub_file_chapters(self):
        ...
        if file_management.file_exists(epub_file_path):
            ...
            self._log(f"{i18n._('loading')} {epub_file_path} ...")
            ...
            self._load_and_mark_completed_chapters()

            self._log(i18n._("loaded"))
    ...
    def _select_all_chapters(self):
        ...
        self._log(i18n._("selection_done"))

    def _unselect_all_chapters(self):
        for i in range(self.chapter_listbox.size()):
            self.chapter_listbox.deactivate(i)

    def _mark_chapter_completed_in_listbox(self, chapter_file_name):
        for i in range(self.chapter_listbox.size()):
            if self.chapter_listbox.get(i) == chapter_file_name:
                self.chapter_listbox.activate(i)
                self.chapter_completed.add(chapter_file_name)
                self._refresh_chapter_colors()

    def _load_and_mark_completed_chapters(self):
        if not self.epub_file_path.get():
            return
        progress = progress_tracker.load_progress(self.epub_file_path.get())
        for chapter_file_name in progress:
            if progress[chapter_file_name].get("completed", False):
                self.chapter_completed.add(chapter_file_name)
        self._refresh_chapter_colors()

    def _refresh_chapter_colors(self):
        for button in self.chapter_listbox.buttons.values():
            chapter_name = button.cget("text")
            if chapter_name in self.chapter_completed:
                button.configure(text_color="#FF5555")
            else:
                button.configure(text_color=self.chapter_listbox.text_color)

    def _reset_progress(self):
        if self.epub_file_path.get():
            progress_tracker.reset_progress(self.epub_file_path.get())
            self._log(i18n._("progress_reset"))
            self.chapter_completed = set()
            self.load_epub_file_chapters()
